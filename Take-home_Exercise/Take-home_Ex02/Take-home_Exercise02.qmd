...
# Step 2: Get all works Sailor contributed to (songs or albums)
sailor_works <- edges_tb1 %>%
  filter(source == sailor_id, `Edge Type` %in% collab_types) %>%
  pull(target)

# Step 3: Find collaborators (distinct person per work)
collaborator_summary <- edges_tb1 %>%
  filter(target %in% sailor_works,
         `Edge Type` %in% collab_types,
         source != sailor_id) %>%
  distinct(source, target) %>%  # ensure 1 collaboration per work per person
  count(source, name = "collab_count") %>%
  left_join(nodes_tb1, by = c("source" = "id")) %>%
...
collaborator_summary %>%
  slice_head(n = 10) %>%
  knitr::kable(caption = "Top 10 Collaborators with Sailor Shift (Unique per Work)")
...
Each arrow points from a collaborator to Sailor Shift, indicating they worked together on at least one unique musical work. The graph consolidates multiple roles per person per work to avoid inflating collaboration counts.
...
# 4. 找出合作人（每人每作品一次）
collab_edges_unique <- edges_tb1 %>%
  filter(target %in% sailor_works,
         `Edge Type` %in% collab_types,
         source != sailor_id) %>%
  distinct(source, target)

# 5. 构建节点（包括 Sailor 和合作人）
collaborator_ids <- unique(collab_edges_unique$source)

nodes_vis <- nodes_tb1 %>%
  filter(id %in% c(sailor_id, collaborator_ids)) %>%
  mutate(
    id = as.character(id),
    label = name,
    group = ifelse(id == sailor_id, "Sailor", "Collaborator"),
    title = paste0("<b>", name, "</b><br>Type: ", `Node Type`)
  ) %>%
  select(id, label, group, title)

# 6. 构建边（合作人 → Sailor，表示他们共同参与作品）
edges_vis <- collab_edges_unique %>%
  mutate(
    from = as.character(source),
    to = as.character(sailor_id),
    arrows = "to",                 # 显示箭头
    label = "collaborated on",    # 可选，添加边的文字说明
    color = "#888888"             # 灰色边
  ) %>%
  select(from, to, arrows, label, color)
...
## 7.4 `visNetwork` Visualization of Sailor Shift’s Collaborators and Direct/Indirect Influence
...
collaborators <- nodes_tb1 %>% 
  filter(id %in% collaborator_edges$source)
...
collab_influence_targets <- nodes_tb1 %>%
  filter(id %in% collab_influence_edges$target)
...
oceanus_targets <- collab_influence_targets %>%
  filter(str_to_lower(genre) == "oceanus folk")

```

### Step 4: Plot the `visNetwork` graph
